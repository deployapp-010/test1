...
#             components.html(source_code, height = 200)
    ...
    with col1:
        ...
        st.table(demo[demo['Demographics'].isin(['Age','Gender(Male)'])].reset_index(drop=True).style.format({"Archetype 1": "{:.1f}%", "Archetype 2": "{:.1f}%","Archetype 3": "{:.1f}%","Archetype 4": "{:.1f}%"}, subset=pd.IndexSlice[demo[demo['Demographics']=='Gender(Male)'].index, :]).format({"Archetype 1": "{:.1f}", "Archetype 2": "{:.1f}","Archetype 3": "{:.1f}","Archetype 4": "{:.1f}"}, subset=pd.IndexSlice[demo[demo['Demographics']=='Age'].index, :]).background_gradient(cmap=cmap_cf, axis=1).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'black'),])]))

        st.subheader('Comorbidities')
        st.table(demo[~(demo['Demographics'].isin(['Age','Gender(Male)','Gender(Female)']))].reset_index(drop=True).style.format({"Archetype 1": "{:.1f}%", "Archetype 2": "{:.1f}%","Archetype 3": "{:.1f}%","Archetype 4": "{:.1f}%"}).background_gradient(cmap=cmap_cf, axis=1).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'black'),])]))
