...
#             components.html(source_code, height = 200)
    ...
    with col1:
        ...
        st.table(demo[demo['Demographics'].isin(['Age','Gender(Male)'])].reset_index(drop=True).style.format({"Archetype 1": "{:.1f}%", "Archetype 2": "{:.1f}%","Archetype 3": "{:.1f}%","Archetype 4": "{:.1f}%"}, subset=pd.IndexSlice[demo[demo['Demographics']=='Gender(Male)'].index, :]).format({"Archetype 1": "{:.1f}", "Archetype 2": "{:.1f}","Archetype 3": "{:.1f}","Archetype 4": "{:.1f}"}, subset=pd.IndexSlice[demo[demo['Demographics']=='Age'].index, :]).background_gradient(cmap=cmap_cf, axis=1).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'white'),])]))

        st.subheader('Comorbidities')
        st.table(demo[~(demo['Demographics'].isin(['Age','Gender(Male)','Gender(Female)']))].reset_index(drop=True).style.format({"Archetype 1": "{:.1f}%", "Archetype 2": "{:.1f}%","Archetype 3": "{:.1f}%","Archetype 4": "{:.1f}%"}).background_gradient(cmap=cmap_cf, axis=1).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'white'),])]))
    ...
    #             st.table(data.head())
        ...
        #         distribution = distribution.sort_values(by=str(sort) ,ascending=False).reset_index(drop=True)
                ...
                # Inject CSS with Markdown
                st.markdown(hide_table_row_index, unsafe_allow_html=True)            
                st.dataframe(distribution.style.format({"Archetype 1": "{:.1f}%", "Archetype 2": "{:.1f}%","Archetype 3": "{:.1f}%","Archetype 4": "{:.1f}%", "Overall": "{:.1f}%"}).background_gradient(cmap=cmap_cf, axis=1, subset=distribution.filter(regex='Archetype').columns.tolist()).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'white'),])]), height=500)
            with col2:
    ...
    #         distribution = distribution.sort_values(by=str(sort) ,ascending=False).reset_index(drop=True)
            ...
            st.dataframe(distribution.style.format({"Archetype 1": "{:.1f}", "Archetype 2": "{:.1f}","Archetype 3": "{:.1f}","Archetype 4": "{:.1f}", "Overall": "{:.1f}", "Standard Deviation": "{:.2f}"})
                    .background_gradient(cmap=cmap_cf, axis=1, subset=distribution.filter(regex='Archetype').columns.tolist()).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'white'),])]), height=500, width=1500)
    ...
    #         distribution = distribution.sort_values(by=str(sort) ,ascending=False).reset_index(drop=True)
            ...
            st.markdown(hide_table_row_index, unsafe_allow_html=True)
            st.dataframe(distribution.style.format({"Archetype 1": "{:.1f}", "Archetype 2": "{:.1f}","Archetype 3": "{:.1f}","Archetype 4": "{:.1f}", "Overall": "{:.1f}", "Standard Deviation": "{:.2f}"})
                    .background_gradient(cmap=cmap_cf, axis=1, subset=distribution.filter(regex='Cluster').columns.tolist()).set_table_styles([dict(selector="th", props=[('font-size', '12px'),('color', 'white'),])]), height=500, width=1500)
